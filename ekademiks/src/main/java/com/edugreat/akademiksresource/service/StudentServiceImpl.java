...
//implementation for the StudentService interface which declare contracts for the Students
@Service
public class StudentServiceImpl implements StudentService {
	...
	//return the questions associated with the test if test exists, else return null
	@Override
	public Collection<Question> takeTest(int testId) {
		...
		//TODO: Throw exception if optional is empty
		return null;
	}

	
	@Override
	public List<ScoreAndDate> getTestScore(int studentId, int testId) {
	...
	public void submitTest(AttemptUtil attempt) {
		...
		//check that the student and test exist
		if(exist(studentId, testId)) {
			
			//get the student who took the test
			Student student = studentDao.findById(testId).get();
			...
			//create new StudentTest object to associate the records with and return the object
			StudentTest studentTest = new StudentTest(score, now, student, test, response);
			studentTestDao.save(studentTest);
			
		} //TODO: THROW EXCEPTION HERE FOR RECORD NOT FOUND
	...
	//it takes the Question and selected options just to compare the answer field in the question
	//and the corresponding selected option
	private double scoreTest(List<Question> questions, List<String> selectedOptions) {
		
		double score = 0.0;
		
		//map each question in the list their corresponding answer
		List<String> answers = null;
		
		   answers = questions.stream()
				.map(Question::getAnswer).collect(Collectors.toList());
		   
		   //iterate the two records and increment score if they're same, otherwise just increment by zero
		...
		//check options submitted by students if they match with the set of enumerated values. 
		private synchronized void checkResponse(String res){
			
			
			// response must be any of the alphabetic letter(A-E). So only one character is allowed
			try {
				...
				if(res.trim().length() == 1) {
					...
					//if the student did not provide answer to a question, add NILL to show no option selected
