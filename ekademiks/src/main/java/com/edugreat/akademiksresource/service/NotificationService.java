...
public class NotificationService {
    ...
    @Transactional
	public void postNotification(NotificationRequestDTO dto) {
		

		
		Notification newNotification = mapToNotification(dto);
...
//			Get the just added notification
			Notification currentNotification = notificationDao.findByCreatedAt(createdAt);
...
//			Instantly notify the intended students who are currently logged in
			targetedStudents.forEach(student -> notificationManager.sendNotification(currentNotification, student.getId()));
    ...
    private Notification mapToNotification(NotificationRequestDTO dto) {
    	
    	return  new Notification(dto.getType(), dto.getMetadata(), dto.getMessage());
